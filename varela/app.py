...
def approximate_solution(inputFile, verbose=False, log=False, count=False, bruteForce=False, approximation=False):
    ...
    sparse_matrix = parser.read(inputFile)
...
def main():
    ...
    helper = argparse.ArgumentParser(prog="approx", description='Estimating the Minimum Vertex Cover with an approximation factor of ≤ 3/2 for an undirected graph encoded in DIMACS format and stored in a file.')
